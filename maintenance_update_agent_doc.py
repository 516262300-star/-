...
def build_blocks() -> list[dict]:
    return [
        ...
        bullet("Notion 写入阶段会显示读取字段、读取已有数据、写入第几行；如果网络断开，会显示重试原因。"),
        heading(2, "2. 日常怎么用"),
        paragraph("正常情况："),
        ...
        bullet("Notion API 请求会优先使用系统代理，并在 SSL/连接中断时重试。"),
        ...
        bullet("notion_sync.py：Notion 字段映射、去重、新建、更新、失败提醒、网络重试和创建后反查。"),
